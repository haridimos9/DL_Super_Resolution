...
def main():
    ...
    dataloader = DataLoader(
        ImageDataset(opt.data_root_folder,
                     hr_shape=hr_shape, resize=opt.resize_hr, downsampling=opt.downsampling, frames_per_sample=opt.frames_per_sample),
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        for i, imgs in enumerate(dataloader):
            ...
            # Content loss
            # Since VGG is pretrained with cnv2d, this is the best I can do for now
            gen_features = feature_extractor(gen_hr[:, :, 0, :, :])
            real_features = feature_extractor(imgs_hr[:, :, 0, :, :])
            ...
            print(f"SR image shape: {gen_hr.detach()[0].shape}")
            print(f"HR image shape: {imgs_hr[0].shape}")
            ...
            if batches_done % opt.sample_interval == 0:
                ...
                imgs_lr = nn.functional.interpolate(
                    imgs_lr[:, :, 0, :, :], scale_factor=4)
                print(f"Images lr shape 1: {imgs_lr.shape}")
                gen_hr = make_grid(gen_hr, nrow=1, normalize=True)
                imgs_lr = make_grid(
                    imgs_lr, nrow=1, normalize=True)
