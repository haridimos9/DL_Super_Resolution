...
parser = argparse.ArgumentParser(
    description="Train iSeeBetter: Super Resolution Models"
)
parser.add_argument(
...
parser.add_argument(
    "--start_epoch", type=int, default=1, help="Starting epoch for continuing training"
)
parser.add_argument(
    "--nEpochs", type=int, default=70, help="number of epochs to train for"
)
...
parser.add_argument(
    "--other_dataset", type=bool, default=False, help="use other dataset than vimeo-90k"
)
parser.add_argument("--future_frame", type=bool, default=True, help="use future frame")
parser.add_argument("--nFrames", type=int, default=7)
parser.add_argument(
    "--patch_size", type=int, default=64, help="0 to use original frame size"
)
parser.add_argument("--data_augmentation", type=bool, default=True)
parser.add_argument("--model_type", type=str, default="RBPN")
parser.add_argument("--residual", type=bool, default=False)
parser.add_argument(
    "--pretrained_sr", default="RBPN_4x.pth", help="sr pretrained base model"
)
...
parser.add_argument(
    "-v", "--debug", default=False, action="store_true", help="Print debug spew."
)
parser.add_argument(
    "-n", "--name", type=str, default="debug", required=False, help="Name of neptune Run."
)
...
def trainModel(
    ...
    for data in iterTrainBar:
        ...
        run["training/DLoss"].log(DLoss.item())
        run["training/GLoss"].log(GLoss.item())
...
def saveModelParams(epoch, runningResults, netG, netD, data_frame, args):
    results = {
        "DLoss": [],
        "GLoss": [],
        "DScore": [],
    ...
    torch.save(
        netG.state_dict(), "weights/netG_epoch_%d_%d.pth" % (UPSCALE_FACTOR, epoch)
    )
    torch.save(
        netD.state_dict(), "weights/netD_epoch_%d_%d.pth" % (UPSCALE_FACTOR, epoch)
    )

    logger.info(
        "Checkpoint saved to {}".format(
            "weights/netG_epoch_%d_%d.pth" % (UPSCALE_FACTOR, epoch)
        )
    )
    logger.info(
        "Checkpoint saved to {}".format(
            "weights/netD_epoch_%d_%d.pth" % (UPSCALE_FACTOR, epoch)
        )
    )

    netG_epoch = "netG_epoch_%d_%d.pth" % (UPSCALE_FACTOR, epoch)
    netD_epoch = "netD_epoch_%d_%d.pth" % (UPSCALE_FACTOR, epoch)

    run[f"io_files/artifacts/{netG_epoch}"].upload(
        "weights/netG_epoch_%d_%d.pth" % (UPSCALE_FACTOR, epoch)
    )

    run[f"io_files/artifacts/{netD_epoch}"].upload(
        "weights/netD_epoch_%d_%d.pth" % (UPSCALE_FACTOR, epoch)
    )
    ...
    if epoch % 1 == 0 and epoch != 0:
        ...
        data_frame.to_csv(
            out_path + str(args.name) + "_epoch_" + str(epoch) + "_scale_" + str(UPSCALE_FACTOR) + "_TrainResults.csv",
            index_label="Epoch",
        )
...
def main():
    ...
    apiToken = os.environ["NEPTUNE_API_TOKEN"]
    project = os.environ["NEPTUNE_PROJECT"]

    logger.info("==> Setting Neptune")
    run = neptune.init(
        project=project,
        api_token=apiToken,
        name=args.name,
    ...
    if args.pretrained:
        ...
        utils.loadPreTrainedModel(
            gpuMode=args.gpu_mode, model=netG, modelPath=modelPath
        )
    ...
    for epoch in range(args.start_epoch, args.nEpochs + 1):
        ...
        if (epoch + 1) % (args.snapshots) == 0:
            saveModelParams(epoch, runningResults, netG, netD, data_frame, args)

    # model architecture
    with open(f"./weights/architecture/arch_netG.txt", "w") as f:
        f.write(str(netG))

    with open(f"./weights/architecture/arch_netD.txt", "w") as f:
        f.write(str(netD))

    run[f"io_files/artifacts/arch_netG"].upload(f"./weights/architecture/arch_netG.txt")

    run[f"io_files/artifacts/arch_netD"].upload(f"./weights/architecture/arch_netD.txt")
